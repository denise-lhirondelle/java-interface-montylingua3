...
import MontyLingua
from jmontylingua3.interfaces import JMontyLinguaInterface

class JMontyLingua(JMontyLinguaInterface):
    ...
    def jist(self,text):
        "@sig public String jist(String text)"
        svoos_list = self.theMontyLingua.jist(text)
        return '\n\n'.join(map(lambda x:'\n'.join(x),svoos_list))
